package com.czht.smartpark.common.core.support;

import com.czht.smartpark.common.base.dto.BaseTree;
import com.czht.smartpark.common.util.PubUtil;
...
public abstract class AbstractTreeService<T extends BaseTree<T, ID>, ID extends Serializable> implements ITree<T, ID> {
	...
	public List<T> getChildList(List<T> list, T t) {
		...
		for (T child : list) {
			if (PubUtil.isEmpty(child.getPid())) {
				continue;
			}
			// 判断集合的父ID是否等于上一级的id
